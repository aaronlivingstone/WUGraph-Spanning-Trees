...
/**
 *  A VList is a mutable doubly-linked list ADT.  Its implementation is
...
public class VList {
  ...
  protected int size;
  protected VListNode head;

  /* VList invariants:
   *  1)  head != null.
   *  2)  For every VListNode x in a VList, x.next != null.
   *  3)  For every VListNode x in a VList, x.prev != null.
   *  4)  For every VListNode x in a VList, if x.next == y, then y.prev == x.
   *  5)  For every VListNode x in a VList, if x.prev == y, then y.next == x.
   *  6)  For every VList l, l.head.myList = null.  (Note that l.head is the
   *      sentinel.)
   *  7)  For every VListNode x in a VList l EXCEPT l.head (the sentinel),
   *      x.myList = l.
   *  8)  size is the number of VListNodes, NOT COUNTING the sentinel,
   *      that can be accessed from the sentinel (head) by a sequence of
   *      "next" references.
   **/

  /**
   *  newNode() calls the VListNode constructor.  Use this method to allocate
   *  new VListNodes rather than calling the VListNode constructor directly.
   *  That way, only this method need be overridden if a subclass of VList
   ...
   **/
  protected VListNode newNode(Object item, VList list, VListNode prev, VListNode next) {
    return new VListNode(item, list, prev, next);
  }

  /**
   *  VList() constructs for an empty VList.
   **/
  public VList() {
  ...
  /**
   *  insertFront() inserts an item at the front of this VList.
   *
  ...
  public void insertFront(Object item) {
    // Your solution here.  Similar to Homework 4, but now you need to specify
    //   the `list' field (second parameter) as well.
      VListNode temp = newNode(item, this, head, head.next);
      head.next.prev = temp;
      head.next = temp;
      size++;
  ...
  /**
   *  insertBack() inserts an item at the back of this VList.
   *
   *  @param item is the item to be inserted.
   *
   *  Performance:  runs in O(1) time.
  ...
  public void insertBack(Object item) {
    // Your solution here.  Similar to Homework 4, but now you need to specify
    //   the `list' field (second parameter) as well.
      VListNode temp = newNode(item, this, head.prev, head);
      head.prev.next = temp;
      head.prev = temp;
      size++;
  }
  ...
  /**
   *  front() returns the node at the front of this VList.  If the VList is
   *  empty, return an "invalid" node--a node with the property that any
   *  attempt to use it will cause an exception.  (The sentinel is "invalid".)
   *
   *  DO NOT CHANGE THIS METHOD.
   *
   *  @return a ListNode at the front of this VList.
   *
   *  Performance:  runs in O(1) time.
   */
  public VListNode front() {
    return head.next;
  }

  /**
   *  back() returns the node at the back of this VList.  If the VList is
   *  empty, return an "invalid" node--a node with the property that any
   *  attempt to use it will cause an exception.  (The sentinel is "invalid".)
   *
   *  DO NOT CHANGE THIS METHOD.
   *
   *  @return a ListNode at the back of this VList.
   *
   *  Performance:  runs in O(1) time.
   */
  public VListNode back() {
    return head.prev;
  }

  /**
   *  toString() returns a String representation of this VList.
   *
   *  DO NOT CHANGE THIS METHOD.
   *
   *  @return a String representation of this VList.
   *
   *  Performance:  runs in O(n) time, where n is the length of the list.
   */
  public String toString() {
    String result = "[  ";
    VListNode current = head.next;
    while (current != head) {
  ...
  private static void testInvalidNode(VListNode p) {
